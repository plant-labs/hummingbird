-- Bubble color driver: release outcome (captive vs released/rescued)

ALTER TABLE geo_agg_cache
  ADD COLUMN IF NOT EXISTS dominant_outcome TEXT;
...
BEGIN
  ...
  INSERT INTO geo_agg_cache (
    geo_id, level, name, state, lga, lat, lng, count, intensity, by_type,
    dominant_verification, dominant_outcome
  )
  SELECT
    CASE
    ...
    CASE
      WHEN BOOL_OR(i.current_status::text NOT IN ('released', 'rescued')) THEN 'captive'
      ELSE 'resolved'
    END
  FROM incidents i
